...
export const TokestrationPanel = ({ checkout }: Props) => {
  ...
  const [psp, setPsp] = useState<Provider>('braintree');
  const [universalToken, setUniversalToken] = useState<UniversalToken>();
  const [chargeResponse, setChargeResponse] = useState<ChargeResponse>();

  const checkoutTotal =
    (checkout?.autoPolicyValue || 0) + (checkout?.homePolicyValue || 0);

  const canCharge = universalToken?.providers?.[psp]?.token;
            ...
            <>
              ...
              <Box alignItems="center" display="flex" whiteSpace="pre">
                ...
                <FormControl>
                  <Select
                    label="Processor"
                    onChange={(e) => setPsp(e.target.value as Provider)}
                    value={psp}
                    variant="standard"
                  >
